// The Pinkeye eye orb, as a framework-free ES module.
//
// This is a direct port of web/app/EyeOrb.tsx (the operator UI's React component) with the React
// wrapper stripped off — the three.js scene, textures and gaze dynamics are identical, so both
// renders of the eye stay the same creature. Keep the two in sync when either changes.
//
// A real 3D eyeball (not a flat icon), freshly plucked from its socket — it trails a fleshy
// optic-nerve stalk behind it — that tracks the cursor.
//
// Scene graph:
//   scene
...
//   │  └─ iris / pupil / cornea (flat discs + clear clearcoat dome)
...
// Rendered larger than the 160px mount so the orb overflows it, staying round. The camera is pulled
// back to z=5.2 to open up enough frustum below and behind the ball for the optic-nerve tail to
// droop into view without being cropped.
const SIZE = 310;
...
// pivot as far as the ball it hangs off — which also keeps the swinging tip inside the frustum.
...
// against the gaze each frame. The max nudge, sin(MAX_DEFLECTION)·IRIS_RECESS ≈ 0.056, must stay
// under the ~0.08 gap between the iris disc (r=0.42) and the cornea rim (r≈0.5) so the iris never
// slides out from under the dome.
...
// capillary tip or reaches the iris zone. Vessels are grown first (geometry only) and stroked
// afterwards, so the same paths can be drawn twice: blurred/dark underneath, sharp on top — that
// under-glow is what makes canvas veins read as *in* the tissue instead of painted on it.
function growVessel(size, x, y, angle, width, segs) {
  let px = x;
  let py = y;
  let a = angle;
  let w = width;
...
function strokeVesselPath(ctx, s) {
  ctx.beginPath();
...
function strokeVessels(ctx, segs) {
  ctx.lineCap = "round";
  ctx.save();
  ctx.filter = "blur(2px)";
  for (const s of segs) {
...
// The sclera's base texture: near-white with a faint red flush and the fine capillary web. The
// heavy "infected" elements — the strong red bloom and the thick engorged trunk vessels — live on
// the separate pulse-layer texture below so their opacity can throb; the faint bloom kept here
// guarantees the eye never looks healthy even at the pulse's minimum.
function makeScleraTexture() {
  ...
  // texture's center lands on the front of the ball where the iris sits. Keep the base close to
  // true white — the "infected" read needs to come from clearly red veins/bloom against white, not
  // from tinting the whole sclera tan/beige (that reads as an off-color material, not redness).
...
// The iris, in the brand-pink family with real anatomical structure: a multi-stop radial gradient
// shifting across several pink/magenta tones (not one flat pink), angular color variation, a
// lighter collarette ring around the pupil, dense radial fibers in both dark and light, and — the
// key "reads as human" cue — a dark limbal ring baked in at the outer edge.
function makeIrisTexture() {
  ...
  for (let i = 0; i < fiberCount; i++) {
    ...
    ctx.strokeStyle = Math.random() > 0.55
...
// Mounts the orb into `container`. Returns a dispose() that tears the scene down again; on a
// machine with no WebGL it falls back to the flat ◉ glyph and returns a no-op.
export function mountEyeOrb(container) {
  if (!container) return () => {};
  ...
  try {
    ...
    fallback.className = "eye-btn-icon";
  ...
  // Distance chosen so both the radius-1 ball and the tail clear the frustum. At FOV 35°/z=5.2 the
  // frustum half-height is ~1.64 at the ball's equator and ~1.81 at the tail tip's depth — enough
  // for the stalk to droop to y≈-1.45 and still swing with the gaze un-cropped.
  ...
  // Default SphereGeometry UVs put the texture's horizontal center (u=0.5) on +X — the right side
  // of the ball from the camera. Rotating the geometry brings u=0.5 around to +Z (the camera/iris
  // side), so vessels originate at the back and grow forward toward the limbus, and the UV seam
  // hides behind the ball. The pulse shell gets the same re-aim so both layers stay registered.
  scleraGeometry.rotateY(-Math.PI / 2);
  const scleraMaterial = new THREE.MeshStandardMaterial({ map: scleraTexture, roughness: 0.28, metalness: 0 });
  const sclera = new THREE.Mesh(scleraGeometry, scleraMaterial);

  // Throbbing redness: bloom + trunk vessels on a shell a hair outside the sclera, opacity animated
  // on a heartbeat in the render loop. depthWrite off so it blends over the sclera without fighting
  // it; the opaque iris/pupil discs sit outside this radius and depth-occlude it.
  const pulseTexture = makePulseTexture();
  const pulseGeometry = new THREE.SphereGeometry(1.002, 64, 64);
  pulseGeometry.rotateY(-Math.PI / 2);
  ...
  const irisMaterial = new THREE.MeshStandardMaterial({ map: irisTexture, roughness: 0.42, metalness: 0 });
  ...
  // cornea *meshes* (not the group) past z=1 and rotating the *group* around the origin means every
  // point on those meshes keeps a world-space distance from the origin of at least its offset,
  // regardless of rotation angle, so nothing ever dips back inside the sclera.
  ...
  // Corneal bulge: a clear, glassy spherical cap doming over the iris — the biggest 3D cue. We use
  // MeshPhysicalMaterial with a strong clearcoat specular lobe + very low base opacity (rather than
  // true `transmission`) on purpose: transmission needs an extra opaque render pass into a render
  // target, which is fragile against this canvas's transparent background — clearcoat gives the
  // same wet-glass bulge/highlight/depth read while rendering reliably over a transparent clear
  // color. ior 1.376 is the real cornea's index of refraction (drives the Fresnel edge brightening).
  const corneaGeometry = new THREE.SphereGeometry(0.85, 48, 32, 0, Math.PI * 2, 0, 0.63);
  ...
  // The whole ball rotates as one — sclera included — so the vein texture (and the tail behind it)
  // follows the gaze instead of the iris sliding over a frozen sphere.
  const eyeGroup = new THREE.Group();
  eyeGroup.add(sclera, pulseShell, iris, pupil, cornea);
  scene.add(eyeGroup);

  // The tail pivots at the origin like the eye but is NOT parented to eyeGroup: its rotation chases
  // the eye's at a slower ease in the render loop, so it drags behind gaze changes and sways on its
  // own — secondary motion without bones or physics.
  const tailTexture = makeNerveTexture();
  const tailGeometry = makeTailGeometry();
  const tailMaterial = new THREE.MeshStandardMaterial({ map: tailTexture, roughness: 0.45, metalness: 0 });
  ...
  function animate() {
    ...
    if (now - lastMouseMove < IDLE_MS) {
      ...
      // below center (mouseTarget.y > 0, since browser Y grows downward) is what's wanted.
    ...
    // Two-speed ease: a big target jump triggers the fast saccade snap, which sticks until the eye
    // is nearly on target, then drops back to the slow fixation settle.
    ...
    // Fake iris-recess parallax: a real iris sits ~3mm behind the cornea, so it visibly lags inside
    // the glassy dome when the eye turns. Positive rotation.y swings the front toward +X, so a
    // deeper point lags toward -X; positive rotation.x looks down, so a deeper point lags upward.
  ...
  return () => {
    ...
    for (const d of [
      scleraGeometry, scleraMaterial, scleraTexture,
      pulseGeometry, pulseMaterial, pulseTexture,
      irisGeometry, irisMaterial, irisTexture,
      pupilGeometry, pupilMaterial,
      corneaGeometry, corneaMaterial,
      tailGeometry, tailMaterial, tailTexture,
    ]) d.dispose();
    renderer.dispose();
